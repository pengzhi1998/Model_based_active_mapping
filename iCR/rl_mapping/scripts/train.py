...
def train(params_filepath: str):
    ...
    if params['algorithm'] == 'DDPG':
        ...
        model = DDPG('MultiInputPolicy', env, action_noise=action_noise, policy_kwargs=policy_kwargs, verbose=1, tensorboard_log=params['tensorboard_folder'], gamma=params['gamma'], learning_rate=params['lr'], seed=params['seed'], **params['ddpg'])
    elif params['algorithm'] == 'PPO':
        model = PPO('MultiInputPolicy', env, policy_kwargs=policy_kwargs, verbose=1, tensorboard_log=params['tensorboard_folder'], gamma=params['gamma'], learning_rate=params['lr'], seed=params['seed'], **params['ppo'])
    else:
